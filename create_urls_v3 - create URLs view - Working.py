...
from secrets import api_key, team_id, list_id2, yourls_signature, view_create_urls
import urllib3
import json
...
def get_all_tasks() -> List[Dict]:
    ...
    page = 0
    while True:
        #url = f"https://api.clickup.com/api/v2/list/{list_id2}/task?page={page}"  #this url gets all tasks in a list rather than all tasks in a view.
        url = f"https://api.clickup.com/api/v2/view/{view_create_urls}/task?page={page}"
        response = requests.get(url, headers=headers)
        response_json = response.json()
        #print(f"Response JSON: {response_json}")  # Add this line to print the response JSON
